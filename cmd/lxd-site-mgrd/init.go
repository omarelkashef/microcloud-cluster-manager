...
func InitialiseConfigOIDC(ctx context.Context, clusterState microState.State, siteManagerState *state.SiteManagerState) error {
	return clusterState.Database().Transaction(ctx, func(ctx context.Context, tx *sql.Tx) error {
		...
		err = api.UpdateDaemonOIDCConfig(configs, siteManagerState, clusterState)
		return err
	})
}
