// Package lxd-cluster-mgr provides the main client tool.
package main
...
func main() {
	...
	app := &cobra.Command{
		Use:               "lxd-cluster-mgr",
		Short:             "Command for managing the LXD Cluster Manager daemon",
	...
	var cmdRemoteCluster = cmdRemoteCluster{common: &commonCmd}
	app.AddCommand(cmdRemoteCluster.command())

	var cmdConfig = cmdConfig{common: &commonCmd}
	app.AddCommand(cmdConfig.command())

	var cmdRemoteClusterJoinToken = cmdRemoteClusterJoinToken{common: &commonCmd}
	app.AddCommand(cmdRemoteClusterJoinToken.command())
