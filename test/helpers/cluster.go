...
// InitSingleMemberCluster initializes a single member cluster for testing site manager.
func InitSingleMemberCluster(env *Environment, t *testing.T) {
	...
	daemonBinary := filepath.Join(env.RootDir(), "lxd-site-mgrd")
	cliBinary := filepath.Join(env.RootDir(), "lxd-site-mgr")
